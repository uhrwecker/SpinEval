...
def evaluate(s, rem, fp_to_redshift, fp_to_new_redshift):
    ...
    np.savetxt(fp_to_new_redshift + 'polarization.csv', redshift_data, delimiter=',', header='alpha,beta,redshift')

    return np.nanmin(result_1), np.mean(result_2), np.nanmean(result_3)


def main(s, fp_to_data, fp_to_save):
    ...
    for n, file in enumerate(phis):
        ...
        if not os.path.isdir(fp_to_save+phi):
...
if __name__ == '__main__':
    s = -0.00175  # 9
    fp_to_data = '/media/jan-menno/T7/Schwarzschild/higher_resolution/redshift_dist_0_sphere/s0/'
    fp_to_save = '/media/jan-menno/T7/Schwarzschild/higher_resolution/pol_angle_0_sphere_2/s-0175/'
    main(s, fp_to_data, fp_to_save)
